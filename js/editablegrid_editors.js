...
DateCellEditor.prototype.getEditor = function(element, value)
{
    // create and initialize text field
    var htmlInput = document.createElement("input");
    htmlInput.setAttribute("type", "date");
    if (this.maxLength > 0) htmlInput.setAttribute("maxlength", this.maxLength);
...
DateCellEditor.prototype.displayEditor = function(element, htmlInput) 
{
    ...
    if (Modernizr.inputtypes.date) {
        $(htmlInput).focusout( function () { this.celleditor.applyEditing(htmlInput.element, htmlInput.value); });
    }
    else {
        $(htmlInput).datetimepicker({
            format: 'Y-m-d',
            timepicker: false,
            closeOnDateSelect: true,
            dayOfWeekStart : 1,
            lang: 'de',
            onClose: function (date) { htmlInput.blur();}
        }).datetimepicker('show');
...
/**
 * Datetimepicker cell editor
 *
 * Text field editor with datetime picker capabilities.
 * Uses the datetimepicker from https://github.com/xdan/datetimepicker
 * This editor is used automatically for datetime columns if we detect that the datetimepicker is present.
 *
 * @constructor Accepts an option object containing the following properties:
 * - fieldSize: integer (default=auto-adapt)
 * - maxLength: integer (default=255)
 *
 * @class Class to edit a cell with a datetimepicker linked to the HTML text input
 */

function DatetimeCellEditor(config)
{
    // erase defaults with given options
    this.init(config);
}
...
DatetimeCellEditor.prototype.getEditor = function(element, value)
{
    // create and initialize text field
    var htmlInput = document.createElement("input");
    htmlInput.setAttribute("type", "datetime-local");
    if (this.maxLength > 0) htmlInput.setAttribute("maxlength", this.maxLength);
    ...
    value = value.replace(/\s/, "T"); // add "T" between date and time so html input accepts the value
    htmlInput.value = this.editorValue(value);
...
// redefine displayEditor to setup datetimepicker
DatetimeCellEditor.prototype.displayEditor = function(element, htmlInput)
{
    // call base method
    TextCellEditor.prototype.displayEditor.call(this, element, htmlInput);

    if (Modernizr.inputtypes['datetime-local']) {
        $(htmlInput).focusout( function () {
            var dateTimeString = htmlInput.value;
            dateTimeString = dateTimeString.replace(/T/, " "); // remove "T" between date and time so string gets accepted by database
            dateTimeString = dateTimeString.match(/[0-9]{4}-[0-9]{2}-[0-9]{2}\s[0-9]{2}:[0-9]{2}/)[0]; // drop seconds and milliseconds if they are in the string
            this.celleditor.applyEditing(htmlInput.element, dateTimeString);
        });
    }
    else {
        $(htmlInput).datetimepicker({
