...
EditableGrid.prototype.parseColumnType = function(column)
{
	...
	if (column.datatype.match(/(.*)\((.*),(.*),(.*),(.*),(.*),(.*)\)$/)) {
		...
		// trim should be done after fetching RegExp matches because it itself uses a RegExp and causes interferences!
					...
					column.datatype == "email" ? new EmailCellRenderer() : 
						column.datatype == "website" || column.datatype == "url" ? new WebsiteCellRenderer() : 
							column.datatype == "date" ? new DateCellRenderer() :
                                column.datatype == "datetime" ? new DatetimeCellRenderer() :
                                    new CellRenderer();
				...
				column.datatype == "boolean" ? null :
					column.datatype == "email" ? new TextCellEditor(column.precision) :
						column.datatype == "website" || column.datatype == "url" ? new TextCellEditor(column.precision) :
							column.datatype == "date" ? (typeof $ == 'undefined' || typeof $.datepicker == 'undefined' ? new TextCellEditor(column.precision, 10) : new DateCellEditor({ fieldSize: 10, maxLength: 10 })) :
                                column.datatype == "datetime" ? (typeof $ == 'undefined' /*|| typeof $.datetimepicker == 'undefined'*/ ? new TextCellEditor(column.precision, 10) : new DatetimeCellEditor({ fieldSize: 16, maxLength: 10 })) :
...
EditableGrid.prototype._addDefaultCellValidators = function(column)
{
    ...
    else if (column.datatype == "datetime") column.cellValidators.push(new DatetimeCellValidator(this));
};
