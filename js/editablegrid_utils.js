...
//
// mm-dd-yyyy hoe: not supported anymore
// mm/dd/yyyy
// mm.dd.yyyy
// mm dd yyyy
// mmm dd yyyy
// mmddyyyy
//
// m-d-yyyy hoe: not supported anymore
// m/d/yyyy
// m.d.yyyy,
// m d yyyy
// mmm d yyyy
//
// // m-d-yy hoe: not supported anymore
// // m/d/yy
// // m.d.yy
// // m d yy,
// // mmm d yy (yy is 20yy) 

// added support for machine date format
// if the date contains a hyphen (-), we assume the following format
// yyyy-mm-dd
...
EditableGrid.prototype.checkDate = function(strDate, strDatestyle) 
{
    ...
    if (strSeparator == "-") {
        // assume machine date format yyyy-mm-dd
        strTemp = strDay;
        strDay = strYear;
        strYear = strTemp;
    }
...
// function that checks if a string complies to datetime format
// yyyy-mm-dd hh:mm
// yyyy-mm-ddThh:mm
EditableGrid.prototype.checkDatetime = function(strDatetime, strDatestyle)
{
    strDatestyle = strDatestyle || this.dateFormat;
    strDatestyle = strDatestyle || "EU";
    ...
    if (!strDatetime || strDatetime.length < 1) return 0;

    strDatetimeArray = strDatetime.split(" ");
    if (strDatetimeArray.length != 2) {
        strDatetimeArray = strDatetime.split("T"); // if there is no space between date and time, try "T"
        if (strDatetimeArray.length != 2) return 1;
    }
    ...
    strTimeArray = strDatetimeArray[1].split(":");
    if (strTimeArray.length != 2) return 1;
    else {
    ...
    if (isNaN(intMinute)) return 2;
    if (intMinute>59 || intMinute<0) return 5;

    // get and check hour
    intHour = parseInt(strHour, 10);
    if (isNaN(intHour)) return 2;
    if (intHour>23 || intHour<0) return 5;
    ...
    return {
        formattedDatetime: (strDatestyle == "US" ? strMonthArray[intMonth-1] + " " + intDay + " " + strYear + " " + strHour + ":" + strMinute : intDay + " " + strMonthArray[intMonth-1] + " " + strYear + " " + strHour + ":" + strMinute),
        sortDate: Date.parse(intMonth + "/" + intDay + "/" + intYear + " " + intHour + ":" + intMinute),
        dbDate: intYear + "-" + intMonth + "-" + intDay + " " + intHour + ":" + intMinute
    };
};
